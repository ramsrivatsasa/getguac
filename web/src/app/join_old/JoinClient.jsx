...
const BLOG_SPOTLIGHT_SLUGS = [
  'track-spending-with-receipts',
  'cancel-unused-subscriptions',
  'get-the-refund-youre-owed',
  'emergency-fund-size',
]
const BLOG_SPOTLIGHT_IMAGES = [
  '/home/story-people/capture-phone.webp',
  '/home/story-people/prepare.webp',
  '/home/story-people/protect.webp',
  '/home/story-people/family-tablet.webp',
]
...
// `showComparison` exists so /join_demo can drop the receipt-vs-statement
// block WITHOUT forking this file. join_demo/page.jsx is deliberately a
// mirror of /join so the two cannot drift; a prop keeps that true, a copy
// would not.
export default function JoinClient({ showComparison = true, receiptFlowVariant = 'default', marketingPolish = false }) {
  ...
  useEffect(() => {
    if (!trialDeleted) return undefined
  ...
  useEffect(() => {
    ...
    if (params.get('try') !== 'receipt') return
    setTryNameOpen(true)
    trackClick(params.get('from') === 'home' ? 'home-try-one-receipt' : 'join-direct-trial')
  ...
  return (
    ...
    <div className={marketingPolish ? 'gj-marketable' : undefined} style={{ background: '#fff', overflowX: 'hidden' }}>
        ...
        <div style={{ maxWidth: 1140, margin: '0 auto' }}>
          ...
          <div className="gj-herorow">
              ...
              <div className="gj-heroauthstack" ref={heroCtaRef}>
                <div className="gj-heroctas">
                  <button
                    ...
                    className={authBtn}
                    style={{
                      background: '#15281C', color: '#fff',
                      padding: '17px 26px', fontSize: 17, fontWeight: 800,
                      maxWidth: 330,
                      boxShadow: '0 14px 28px -12px rgba(21,40,28,0.55)',
                    }}
                  >
            ...
            <div className="gj-hero-phone">
              ...
              <HeroArt marketable={marketingPolish}>
      ...
      <BelowFold>
      {/* Order 2026-08-04, narrative first: the receipt-vs-statement proof
          establishes WHY receipts explain your money, THEN the loop shows how
          GetGuac works it. Feature grids come after. The 4-step block below
          duplicates the 8-stage loop — collapse them once the loop settles. */}
      {showComparison && (<>
      ...
      {/* ── 4b. THE RECEIPT FLOW ──────────────────────────────────────────
          CAPTURE → REMEMBER → SHOP SMARTER → KEEP MORE, with the return loop.
          Built to a supplied design; the markup lives in
          components/ReceiptFlow.jsx so this file only gains one line.
          ⚠️ Overlaps in subject with section 3 ("How GetGuac works in 4 simple
          steps") further down — two four-step explanations now sit on the same
          page. Added rather than swapped because replacing section 3 was not
          what was asked for; collapse them deliberately. */}
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: '30px 20px 6px' }}>
        <ReceiptFlow variant={receiptFlowVariant} />
      </div>
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        /* Supplied hero banner, when web/public/join/hero.png exists.
         ...
         .gj-market-hero { position: relative; width: 100%; max-width: 560px; height: 350px; overflow: visible;
                           margin: 0 auto; background: transparent; border: 0; box-shadow: none; }
         .gj-market-hero:before { display: none; }
         .gj-market-ring { display: none; }
         .gj-market-ring-one { width: 390px; height: 390px; left: 44px; top: 75px; }
         .gj-market-ring-two { width: 240px; height: 240px; right: -100px; top: -75px; border-width: 25px; }
         .gj-market-scene-title { position: absolute; z-index: 7; left: 24px; top: 22px; max-width: 295px;
                                  padding: 11px 15px; border-radius: 16px 16px 16px 5px;
                                  background: rgba(255,255,255,.94); color: #32443A; font-size: 13px; font-weight: 750;
                                  box-shadow: 0 16px 32px -24px rgba(20,50,26,.55); }
         .gj-market-scene-title strong { display: block; margin-top: 2px; color: #315F17;
                                         font-family: var(--font-bricolage), sans-serif; font-size: 18px; }
         .gj-market-person { position: absolute; z-index: 2; overflow: hidden; background: #fff;
                             border: 5px solid rgba(255,255,255,.9); box-shadow: 0 22px 44px -30px rgba(18,55,25,.65); }
         .gj-market-person img { width: 100%; height: 100%; object-fit: cover; display: block; }
         .gj-market-person-main { z-index: 3; left: 0; top: 24px; width: 74%; height: auto; aspect-ratio: 3 / 2; border: 0; border-radius: 24px; }
         .gj-market-person-main img { object-fit: contain; object-position: center; }
         .gj-market-phone { position: absolute; z-index: 2; right: -4%; top: 8px; bottom: auto; width: 46%; height: auto;
                            transform: rotate(2deg); filter: none;
                            -webkit-mask-image: radial-gradient(ellipse 72% 78% at 48% 52%,#000 68%,rgba(0,0,0,.9) 78%,transparent 100%);
                            mask-image: radial-gradient(ellipse 72% 78% at 48% 52%,#000 68%,rgba(0,0,0,.9) 78%,transparent 100%); }
         .gj-market-benefit { position: absolute; z-index: 6; display: block; min-width: 150px; padding: 9px 12px;
                              border-radius: 14px; background: rgba(255,255,255,.95); color: #25382D;
                              border: 1px solid rgba(20,83,45,.12); box-shadow: 0 16px 34px -25px rgba(14,45,20,.6); }
         .gj-market-benefit b, .gj-market-benefit small { display: block; }
         .gj-market-benefit b { font-size: 11px; line-height: 1.25; }
         .gj-market-benefit small { color: #66776C; font-size: 9px; line-height: 1.3; margin-top: 2px; }
         .gj-market-benefit-one { left: 14px; bottom: 28px; }
         .gj-market-benefit-two { right: 12px; top: 202px; }
         .gj-market-benefit-three { right: 14px; bottom: 18px; }
         .gj-market-mascot { position: absolute; z-index: 7; left: 278px; bottom: -5px; width: 86px; height: 104px; }

         .gj-marketable .gj-authbtn:hover { transform: translateY(-2px); }
         .gj-marketable .gj-authbtn:active { transform: translateY(0) scale(.99); }
         .gj-marketable .gj-featurearrow { backdrop-filter: blur(8px); }

         .gj-blogstrip { margin-top: 46px; padding: 52px 24px 58px; background: linear-gradient(180deg,#F8F6EF,#FDFCF8);
                         border-top: 1px solid rgba(20,83,45,.09); border-bottom: 1px solid rgba(20,83,45,.09); }
         .gj-bloginner { max-width: 1180px; margin: 0 auto; }
         .gj-bloghead { display: flex; align-items: flex-end; justify-content: space-between; gap: 30px; margin-bottom: 24px; }
         .gj-bloghead > div > span { display: block; color: #4D7C0F; font-size: 12px; font-weight: 850;
                                    letter-spacing: .11em; text-transform: uppercase; margin-bottom: 9px; }
         .gj-bloghead h2 { margin: 0; color: #15281C; font-family: var(--font-bricolage), sans-serif;
                           font-size: 34px; line-height: 1.08; letter-spacing: -.035em; }
         .gj-bloghead p { margin: 9px 0 0; color: #627168; font-size: 15px; }
         .gj-blogall { flex: 0 0 auto; display: inline-flex; align-items: center; gap: 8px; padding: 11px 17px;
                       border-radius: 999px; background: #15281C; color: #fff; font-size: 13px; font-weight: 800;
                       text-decoration: none; }
         .gj-bloggrid { display: grid; grid-template-columns: repeat(4,1fr); gap: 16px; }
         .gj-blogcard { min-width: 0; overflow: hidden; border-radius: 20px; background: #fff;
                        border: 1px solid rgba(20,83,45,.1); color: inherit; text-decoration: none;
                        box-shadow: 0 18px 42px -36px rgba(18,45,24,.55);
                        transition: transform .18s ease, box-shadow .18s ease, border-color .18s ease; }
         .gj-blogcard:hover { transform: translateY(-4px); border-color: rgba(101,163,13,.32);
                              box-shadow: 0 24px 48px -34px rgba(18,45,24,.6); }
         .gj-blogimage { position: relative; display: block; aspect-ratio: 1.35 / 1; overflow: hidden; background: #EAF3DE; }
         .gj-blogimage img { width: 100%; height: 100%; object-fit: cover; display: block; transition: transform .35s ease; }
         .gj-blogcard:hover .gj-blogimage img { transform: scale(1.035); }
         .gj-blogimage small { position: absolute; left: 12px; top: 12px; padding: 6px 9px; border-radius: 999px;
                               background: rgba(255,255,255,.94); color: #4D7C0F; font-size: 9px; font-weight: 900;
                               letter-spacing: .08em; text-transform: uppercase; }
         .gj-blogbody { display: flex; flex-direction: column; min-height: 194px; padding: 17px 17px 16px; }
         .gj-blogbody strong { color: #15281C; font-family: var(--font-bricolage), sans-serif; font-size: 17px;
                               line-height: 1.22; letter-spacing: -.015em; }
         .gj-blogbody > span { color: #66756C; font-size: 12.5px; line-height: 1.45; margin-top: 8px; }
         .gj-blogbody b { margin-top: auto; padding-top: 15px; color: #4D7C0F; font-size: 12px; }
        ...
        @media (max-width: 1024px) and (min-width: 881px) {
          ...
          .gj-bloggrid { grid-template-columns: repeat(2,1fr); }
        }
        ...
        @media (max-width: 880px) {
          ...
          .gj-market-hero { max-width: 520px; height: 470px; }
          .gj-bloghead { align-items: flex-start; flex-direction: column; gap: 16px; }
          .gj-bloggrid { grid-template-columns: repeat(2,1fr); }
         ...
         @media (max-width: 560px) {
           ...
           .gj-market-hero { height: 270px; overflow: visible; border-radius: 0; }
           .gj-market-person-main { left: 0; top: 18px; width: 76%; height: auto; }
           .gj-market-phone { right: -7%; top: 4px; bottom: auto; width: 47%; }
           .gj-market-benefit { min-width: 128px; padding: 8px 9px; }
           .gj-market-benefit-two { right: 6px; top: 152px; }
           .gj-market-benefit-three { right: 6px; bottom: 9px; }
           .gj-market-benefit-one { display: none; }
           .gj-market-mascot { left: 211px; bottom: -11px; transform: scale(.78); transform-origin: bottom left; }
           .gj-blogstrip { padding: 38px 16px 42px; }
           .gj-bloghead h2 { font-size: 28px; }
           .gj-bloggrid { grid-template-columns: 1fr; }
           .gj-blogbody { min-height: 170px; }
         }
         @media (prefers-reduced-motion: reduce) {
...
function HeroArt({ marketable = false }) {
  ...
  if (marketable) {
    return (
      <div className="gj-market-hero" aria-label="GetGuac turns a receipt into organized, useful shopping intelligence">
        <span aria-hidden className="gj-market-ring gj-market-ring-one" />
        <span aria-hidden className="gj-market-ring gj-market-ring-two" />

        <div className="gj-market-person gj-market-person-main">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src="/home/story-people/family-tablet.webp" alt="A family reviewing their GetGuac savings together" />
        </div>

        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img className="gj-market-phone" src="/home/demo-2/guac-phone-current.webp" alt="GetGuac showing Guacanomics beside an avocado" />
      </div>
    )
  }
...
function JoinBlogStrip() {
  const articles = BLOG_SPOTLIGHT_SLUGS
    .map((slug) => ARTICLES.find((article) => article.slug === slug))
    .filter(Boolean)

  return (
    <section className="gj-blogstrip" aria-labelledby="gj-blog-title">
      <div className="gj-bloginner">
        <div className="gj-bloghead">
          <div>
            <span>Smarter shopping starts here</span>
            <h2 id="gj-blog-title">Small money lessons. Real-life wins.</h2>
            <p>Practical guides to help every receipt teach you something useful.</p>
          </div>
          <Link href="/articles" className="gj-blogall">Explore all guides <span aria-hidden>→</span></Link>
        </div>
        <div className="gj-bloggrid">
          {articles.map((article, index) => (
            <Link key={article.slug} href={`/articles/${article.slug}`} className="gj-blogcard">
              <span className="gj-blogimage">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={BLOG_SPOTLIGHT_IMAGES[index]} alt="" loading="lazy" />
                <small>{article.category}</small>
              </span>
              <span className="gj-blogbody">
                <strong>{article.title}</strong>
                <span>{article.excerpt}</span>
                <b>Read the guide <span aria-hidden>→</span></b>
              </span>
            </Link>
          ))}
        </div>
