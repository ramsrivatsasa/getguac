    ...
    <section className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 sm:p-5">
      <div className="flex items-center justify-between mb-3">
        <button
          ...
          onClick={() => setOpen((o) => !o)}
          aria-expanded={open}
          className="font-semibold text-gray-900 flex items-center gap-2 -m-1 p-1 rounded-lg hover:text-gray-700"
        >
          <Tag size={18} className="text-gray-400" /> Steals for you
          {unread > 0 && (
            <span className="text-[11px] font-extrabold bg-gray-100 text-gray-700 rounded-full px-2 py-0.5">
              {unread} new
            </span>
          )}
          <ChevronDown size={16} className={`text-gray-400 transition-transform ${open ? '' : '-rotate-90'}`} />
        </button>
        <Link href="/steals" className="text-xs font-semibold text-gray-500 hover:text-gray-900 inline-flex items-center gap-1">
      ...
      <div className="space-y-4">
        ...
        {hunting.map((q) => (
          <div key={`hunt-${q}`}>
            <div className="text-[11px] font-bold uppercase tracking-wider text-gray-500 mb-1.5 truncate">{q}</div>
            <Link href="/steals" className="flex items-center gap-2 text-xs text-gray-400 rounded-xl border border-dashed border-gray-200 bg-gray-50/60 px-3 py-3 hover:border-gray-300 hover:text-gray-700 transition-colors">
...
function DealCard({ d }) {
  ...
  return (
    <a
      ...
      className="snap-start shrink-0 w-40 rounded-xl border border-gray-100 bg-white hover:shadow-md hover:border-gray-300 transition overflow-hidden"
    >
