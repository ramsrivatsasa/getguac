...
function WizardAvocado({ size, className = '' }) {
  ...
  return (
    <div className={`relative ${className}`} style={{ width: L, height: H }}>
      ...
      <div className="absolute left-1/2 -translate-x-1/2 pointer-events-none" style={{ top: avoW * 0.06, width: L, height: L }}>
...
export default function FeatureHeader({ theme, expression = 'happy', title, subtitle, badge, action }) {
  return (
    <div className="flex items-center gap-3 sm:gap-4 flex-wrap">
      {THEMED.has(theme)
        ? <ThemedAvocado theme={theme} size={theme === 'wizard' ? 86 : 60} className="shrink-0" />
